...
function Sidebar({
  ...
  // The app's collapse is a drawer (offcanvas): the gap releases its width while
  // the panel slides fully off-canvas. Both are driven by the SAME spring so they
  // stay locked together, and the panel keeps its full width throughout — text
  // never squishes or clips. Reduced motion snaps instantly. Other collapse modes
  // (icon/none) keep the original attribute-driven CSS transitions.
  const spring = reduce
    ? ({ duration: 0 } as const)
      ...
      {collapsible === "offcanvas" ? (
        <>
          <motion.div
            ...
            aria-hidden
            className="relative h-svh bg-transparent group-data-[side=right]:rotate-180"
            initial={false}
            animate={{ width: open ? SIDEBAR_WIDTH : "0rem" }}
            transition={spring}
          />
          <motion.div
            ...
            data-side={side}
            className={cn(
              "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) data-[side=left]:left-0 data-[side=right]:right-0 group-data-[side=left]:border-r group-data-[side=right]:border-l md:flex",
              className
            )}
            initial={false}
